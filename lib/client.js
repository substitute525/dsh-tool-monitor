window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var NO_MONITORS = [];
		/**
		 * Optional primitive icons, resolved once at module load. Icon names differ
		 * across DSH versions, and rendering an undefined component throws
		 * "Element type is invalid" during render — which abdicates this slot entry
		 * (retired from the slot for the rest of its life) and hides the control.
		 */
		var StateDotIcon = _primitives.StateDot != null ? _primitives.StateDot : null;
		var ChevronDownIcon = _primitives.IconChevronDownOutlineRegular != null ? _primitives.IconChevronDownOutlineRegular
			: _primitives.IconChevronDownOutlineMedium != null ? _primitives.IconChevronDownOutlineMedium
			: _primitives.IconChevronDownOutline14 != null ? _primitives.IconChevronDownOutline14
			: null;
		/** Error panel: created only when something actually fails, so a healthy UI stays clean. */
		function diag(text) {
			try {
				if (typeof document === "undefined" || document.body === null) return;
				var el = document.getElementById("tool-monitor-diag");
				if (el === null) {
					el = document.createElement("pre");
					el.id = "tool-monitor-diag";
					el.style.cssText = "position:fixed;right:10px;bottom:10px;z-index:2147483647;max-width:46vw;max-height:34vh;overflow:auto;margin:0;padding:8px 10px;border-radius:8px;background:rgba(127,17,17,.94);color:#ffd9d9;font:11px/15px ui-monospace,monospace;white-space:pre-wrap;pointer-events:none";
					document.body.appendChild(el);
				}
				el.textContent = text;
			} catch (_) { /* diagnostics must never break the plugin */ }
		}
		...
		function MonitorsAction(props) {
			...
			var t = function(key, params) {
				if (typeof rawT !== "function") { diag("tool-monitor: translate helper missing\nkey=" + key); return key; }
				try { return rawT(key, params); } catch (error) { diag("tool-monitor: translate failed\nkey=" + key + "\n" + String((error && error.message) || error)); return key; }
			};
			var ctx = props.__ctx;
			if (!ctx) return null;
			...
			// Poll the list over the harness connection instead of opening a dedicated
			// WebSocket: the desktop app serves the UI from the `dsh-app:` scheme, and
			// `new WebSocket("dsh-app://…")` throws (only http/https/ws/wss are allowed),
			// which crashed this slot entry and hid the control. The connection RPC
			// carries authentication on every platform.
			// Same wire contract over either carrier: the harness Connection RPC when
			// that client service is reachable from this plugin's context, otherwise a
			// same-origin POST to the plugin's own `/tool-monitor/<endpoint>` route.
			// The desktop shell serves the UI from the `dsh-app:` scheme, which IS a
			// registered fetch scheme (so `fetch` works) but NOT a WebSocket scheme —
			// `new WebSocket("dsh-app://…")` throws, and that is what previously
			// crashed this slot entry and hid the control.
			function callHost(endpoint, payload) {
				if (connection !== undefined && connection.rpc !== undefined && typeof connection.rpc.call === "function") {
					try {
						...
						console.warn("[tool-monitor] connection rpc threw, falling back to fetch:", error);
					}
				}
				return fetch(CHANNEL + "/" + endpoint, {
					...
					body: JSON.stringify({
						type: "client-request",
						rpcId: "tool-monitor-" + String(Date.now()) + "-" + Math.random().toString(36).slice(2),
						method: endpoint,
						payload: payload
					})
				}).then(function(res) {
					...
					return envelope !== null && typeof envelope === "object" && envelope.result !== undefined
						? envelope.result
						: { ok: false, error: { code: "internal", message: "malformed response envelope", details: {} } };
				});
			}
			...
			React.useEffect(function() {
				...
				function pull() {
					try {
						callHost("list", { sessionId: sessionId }).then(function(r) {
							if (closed) return;
							if (r && r.ok) setMonitors(r.value || NO_MONITORS);
							else console.warn("[tool-monitor] list poll rejected:", r);
						}).catch(function(e) { console.warn("[tool-monitor] list poll failed:", e); diag("tool-monitor: list poll failed\n" + String((e && e.message) || e)); });
					} catch (e) {
						console.error("[tool-monitor] list poll threw:", e);
					}
				}
				pull();
				timer = setInterval(pull, 2000);
				return function() { closed = true; clearInterval(timer); };
			}, [sessionId, connection]);

			function refreshOutput(id) {
				if (!sessionId) return;
				try {
					callHost("read", { sessionId: sessionId, monitorId: id }).then(function(r) {
						if (r.ok) setOutput(function(o) { var n = Object.assign({}, o); n[id] = { text: r.value.text, lossy: !!r.value.lossy }; return n; });
					}).catch(function() {});
				} catch (e) { console.error("[tool-monitor] read threw:", e); }
			}
			function stopOne(id) {
				if (!sessionId) return;
				try {
					callHost("stop", { sessionId: sessionId, monitorId: id }).then(function(r) {
						if (r.ok) setOutput(function(o) { var n = Object.assign({}, o); n[id] = { text: r.value.text, lossy: !!r.value.lossy }; return n; });
					}).catch(function() {});
				} catch (e) { console.error("[tool-monitor] stop threw:", e); }
			}
			...
			return React.createElement("div", { ref: rootRef, className: "mn-root" },
				React.createElement("button", {
					...
					liveCount > 0 && StateDotIcon !== null ? React.createElement(StateDotIcon, { state: "ongoing", className: "mn-triggerDot" }) : null,
					React.createElement("span", { className: "mn-count" }, countLabel),
					ChevronDownIcon !== null ? React.createElement(ChevronDownIcon, { className: open ? "mn-triggerOpen" : undefined }) : null
				),
		...
		function apply(ctx) {
			// An earlier diagnostic build could leave an error panel and a probe string in the
			// window title behind; clear both on (re)load so the shell owns them again.
			try { var stale = document.getElementById("tool-monitor-diag"); if (stale !== null) stale.remove(); } catch (_) {}
			try { if (String(document.title).indexOf("TM ") === 0) document.title = "DeepSeek Harness"; } catch (_) {}
			ctx.effect(() => ctx.locale.register("monitor", { zh: zh, en: en }), "tool-monitor: dictionaries");
			try {
				var slots = ctx.slots;
				if (slots === undefined) { console.error("[tool-monitor] slots service unavailable"); return; }
				slots.inject("conversation.session.header.actions", function() {
					return slots.register(
						...
						function MonitorsActionSlot(slotProps) {
							try { return React.createElement(MonitorsAction, Object.assign({}, slotProps, { __ctx: ctx })); } catch (renderError) { console.error("[tool-monitor] header action render failed:", renderError); diag("tool-monitor: render failed\n" + String((renderError && renderError.message) || renderError)); return null; }
						}
					);
				});
			} catch (registerError) {
				console.error("[tool-monitor] slot registration failed:", registerError);
				diag("tool-monitor: slot registration failed\n" + String((registerError && registerError.message) || registerError));
			}
		}

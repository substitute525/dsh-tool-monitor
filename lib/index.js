/**
 * `tool-monitor` — background monitors for files, command output, and WebSockets.
 *
 ...
 *
 * Each monitor polls a file (or a background `ShellProcess`, or an outbound
 * `ws` client) and wakes its owning agent with a plugin `notice` whenever
 * triggering output arrives. An idle owner is woken immediately with a single
 * `followup`; while the owner is busy the notices are queued in this plugin and
 * flushed as ONE merged `followup` the moment the owner turns idle, so a busy
 * owner is never interrupted and no notice is dropped. There is deliberately no
 * wake-count limit: the queue is drained by an idle transition rather than by a
 * counter, so it cannot self-excite.
...
export const inject = ['tools', 'systemPrompt', 'fs', 'shell', 'timer', 'connection', 'webServer'];
...
export function apply(ctx, config) {
    ...
    /**
     * Notices that could not be delivered because their owner was busy, keyed by
     * owner. Held here (NOT in the agent inbox) so a cancel/abort cannot discard
     * them and a pre-step batch claim cannot overtake them.
     */
    ...
    let disposeWsRoute;
    /** Disposer returned by `webServer.register` for the RPC route (released on teardown). */
    let disposeRpcRoute;
    ...
    function teardown(mon) {
        ...
        if (mon.socket !== undefined) {
            try {
                mon.socket.terminate();
            }
            catch { /* socket may already be closed */ }
            mon.socket = undefined;
        }
    }
    ...
    /** Build the plugin `notice` UserMessage for one owner. */
    function noticeMessage(text, head) {
        return createUserMessage({
            content: [{ type: 'text', text }],
            source: {
                kind: 'plugin:tool-monitor',
                form: 'notice',
                summary: boundContextSummary(head)
            }
        });
    }
    /** The `monitor <id> (<kind>: <label>)` prefix shared by every notice line. */
    function monPrefix(mon) {
        return `monitor ${mon.id} (${mon.kind}: ${mon.label})`;
    }
    /** Trailing guidance shared by single and merged notices. */
    const NOTICE_FOOTER = 'Read the remains with monitor_read; stop any of them with monitor_stop.';
    /**
     * Deliver one notice to the owner. An idle owner is woken immediately;
     * a busy owner has the notice queued here and flushed as one merged
     * `followup` on the next idle transition. Nothing is ever dropped for
     * lack of wake budget — there is no budget.
     */
    function wakeOwner(mon, body, head) {
        const owner = mon.owner;
        if (owner === undefined) return;
        try {
            if (owner.status === 'idle') {
                owner.followup(noticeMessage(`${monPrefix(mon)} ${body}\n\n${NOTICE_FOOTER}`, head));
                return;
            }
            const queue = pending.get(owner);
            if (queue === undefined) pending.set(owner, [{ mon, body }]);
            else queue.push({ mon, body });
        }
        catch {
    ...
    /**
     * Merge one owner's queued notices into a single `followup`, grouped by
     * monitor. Called when that owner turns idle (and once more at disposal time
     * if the owner still has somewhere to send it).
     */
    function flushPending(owner) {
        ...
        for (const item of queue) {
            const existing = groups.get(item.mon.id);
            if (existing === undefined) groups.set(item.mon.id, { mon: item.mon, bodies: [item.body] });
            else existing.bodies.push(item.body);
        }
        const blocks = [];
        for (const { mon, bodies } of groups.values()) {
            const n = bodies.length;
            blocks.push(`${monPrefix(mon)} — ${n} notice${n === 1 ? '' : 's'} while this session was busy:\n`
                + bodies.map((b) => `• ${b.replace(/\n/g, '\n  ')}`).join('\n'));
        }
        const n = queue.length;
        const head = `${n} queued monitor notice${n === 1 ? '' : 's'} `
            + `from ${groups.size} monitor${groups.size === 1 ? '' : 's'}`;
        const text = `${head} (delivered now that this session went idle):\n\n`
            + `${blocks.join('\n\n')}\n\n${NOTICE_FOOTER}`;
        try {
            owner.followup(noticeMessage(text, head));
        }
        catch {
            // Owner disposed between the idle transition and the flush.
        }
    }
    /** Handle newly observed output: buffer it, test the trigger, wake on a trigger. */
    function emit(mon, delta, force = false) {
        pushText(mon, delta);
        if (mon.patternRe !== null && !force && !mon.patternRe.test(delta)) return;
        const excerpt = excerptTail(delta, noticeLimit);
        if (excerpt.length === 0) return;
        if (mon.oneShot) {
            stopMonitor(mon, 'one-shot');
            wakeOwner(mon, `stopped on first trigger (one-shot):\n${excerpt}`,
                `monitor ${mon.id} (${mon.kind}: ${mon.label}) stopped (one-shot)`);
            return;
        }
        wakeOwner(mon, `produced output:\n${excerpt}`
    ...
    function finalWake(mon, head) {
        ...
        const body = text.length > 0 ? `\n${excerptTail(text, noticeLimit)}` : '';
        wakeOwner(mon, `${statusLine(mon)}${body}`, head);
    }
    ...
    function arm(mon) {
        if (mon.kind === 'file' || mon.kind === 'command') {
    ...
    /**
     * Buffer one spawned child into the process handle shape the poll tick
     * consumes: readOutput() returns { delta, lossy }, alongside status,
     * exitCode, signal and kill().
     */
    function adoptChildProcess(child) {
        let delta = '';
        let errBuf = '';
        let sawStdout = false;
        let lossy = false;
        ...
        // Diagnostics stay off the event stream: stderr is kept (bounded) as
        // the exit story and surfaces only if the run never wrote to stdout.
        child.stderr?.on('data', (chunk) => {
            errBuf += chunk;
            if (errBuf.length > 8192) errBuf = errBuf.slice(-8192);
        });
        child.on('error', (error) => push(`spawn error: ${String(error)}\n`));
        child.on('close', (code, sig) => {
            finished = true;
            exitCode = code ?? null;
            signal = sig ?? null;
            if (!sawStdout && errBuf.length > 0) push(errBuf);
        });
        return {
            get status() { return finished ? 'exited' : 'running'; },
    ...
    /** Spawn one monitor command through the platform shell. */
    function spawnShellProcess(command, cwd) {
        ...
        return adoptChildProcess(process.platform === 'win32'
            ? spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', command], options)
            : spawn(command, { ...options, shell: true }));
    }
    /**
     * Start a background command process for a command monitor.
     *
     * The host shell service is preferred. When the installed DSH no longer
     * exposes its start() process factory, the command is spawned locally so
     * command monitors keep working instead of failing outright.
     */
    function startCommand(mon) {
        try {
            ...
            if (typeof ctx.shell.start === 'function') {
                mon.proc = ctx.shell.start(spec);
                return;
            }
        }
        catch { /* fall through to a locally spawned shell */ }
        try {
            mon.proc = spawnShellProcess(mon.command, mon.cwd);
        }
        catch (error) {
    ...
    /**
     * Start an outbound WebSocket connection for a `ws` monitor. The monitor is
     * event-driven (no poll loop): each text frame is one wake event (a multi-line
     * frame stays whole), a binary frame is reported only as `[binary frame, N
     * bytes]`, a socket error is surfaced before the close, and the close code is
     * shown in the terminal state. Subprotocols (RFC 6455) are offered when given.
     */
    function wsStart(mon) {
        try {
            const socket = mon.protocols !== undefined && mon.protocols.length > 0
                ? new WebSocket(mon.url, mon.protocols)
                : new WebSocket(mon.url);
            mon.socket = socket;
            socket.on('open', () => {
                mon.detail = undefined;
            });
            socket.on('message', (data, isBinary) => {
                ...
                if (isBinary) {
                    const n = Buffer.isBuffer(data) ? data.length : String(data).length;
                    emit(mon, `[binary frame, ${n} bytes]\n`);
                }
                else {
            ...
            socket.on('error', (err) => {
                if (mon.status !== 'running') return;
                emit(mon, `[ws error] ${err && err.message ? err.message : String(err)}\n`, true);
            });
            socket.on('close', (code, reason) => {
                if (isTerminal(mon.status)) return;
                const normal = code === 1000 || code === 1001;
                const detail = `ws closed (code ${code})${reason ? `, ${reason}` : ''}`;
                markDone(mon, normal ? 'completed' : 'failed', detail);
                finalWake(mon, `monitor ${mon.id} (${mon.kind}: ${mon.label}) closed`);
            });
        }
        catch (error) {
            markDone(mon, 'failed', `ws connect failed: ${String(error)}`);
        }
    }
    // -- registry events -------------------------------------------------
    /** Deliver everything queued while this owner was busy, as one merged notice. */
    ctx.on('agent/status', ({ agent, status }) => {
        if (status === 'idle') flushPending(agent);
    });
    ...
    ctx.effect(() => () => {
        ...
        if (disposeRpcRoute !== undefined) {
            try {
                disposeRpcRoute();
            }
            catch { /* route disposer best-effort */ }
            disposeRpcRoute = undefined;
        }
    ...
    ctx.systemPrompt.section({
        name: 'tool:monitor',
        order: 107,
        text: 'Use monitor_start to watch a file, a command output, or a WebSocket (source=ws) in the background; you are woken in-session by a notice when triggering output arrives. For a ws monitor, each text frame is one event (a multi-line frame stays whole), a binary frame is reported only as [binary frame, N bytes], a socket error is surfaced before the close, and the close code is shown in the terminal state. Read the delta with monitor_read (it consumes, like job_output), stop with monitor_stop, list with monitor_list. Do not busy-poll a running monitor. A monitor with one_shot: true stops itself after the first trigger; with pattern set, only matching output triggers it. duration_seconds: 0 keeps it running until monitor_stop or session end.'
    });
    // -- tools -----------------------------------------------------------
    ctx.tools.register(defineTool({
        name: 'monitor_start',
        description: 'Start a background monitor over a file, a command output, or a WebSocket. Returns immediately with a monitor_id; you are woken by an in-session notice when triggering output arrives. duration_seconds: 0 means run until monitor_stop or session end. one_shot: true stops the monitor automatically after the first trigger. pattern (JS regular expression) restricts triggers to matching output (non-matching output stays readable). source=ws connects to a WebSocket: each text frame is one event, a binary frame is reported as [binary frame, N bytes], a socket error surfaces before the close, and the close code is shown in the terminal state.',
        parameters: {
            source: {
                type: 'string',
                enum: ['file', 'command', 'ws'],
            ...
            url: {
                type: 'string',
                description: 'WebSocket URL (required for source=ws), e.g. ws://host:port/path or wss://host/path.'
            },
            protocols: {
                type: 'array',
                items: { type: 'string' },
                description: 'Optional subprotocols to offer per the WebSocket spec (RFC 6455) for source=ws.'
            },
            duration_seconds: {
                type: 'number',
        ...
        async execute(args, exec) {
            ...
            const protocols = Array.isArray(args.protocols) && args.protocols.length > 0
                ? args.protocols.filter((p) => typeof p === 'string')
                : undefined;
            ...
            if (kind === 'ws' && (url === undefined || url.length === 0)) {
                throw new Error('monitor_start: source=ws requires a non-empty url');
            }
            ...
            const label = kind === 'file' ? (path ?? '')
                : kind === 'ws' ? (url.length > 120 ? `${url.slice(0, 119)}…` : url)
    ...
    // on-demand output read and stop (not a polling list). The route is
    // registered directly on the public `webServer.register` seam: the
    // connection service's `.rpc.handle` host API resolves `webServer` through
    // the connection provider's own context, which declares no webServer
    // inject, so it throws `cannot get property "webServer" without inject`
    // during boot on cordis >= 4. The request fence is kept by reusing the
    // connection service's public `requestRejection`, and the wire envelope
    // mirrors the client connection's `client-request` / `server-response`
    // contract (POST {channel}/{endpoint}, JSON body).
    ...
    if (connection !== undefined && webServer !== undefined && typeof webServer.register === 'function') {
        ...
        /** Buffer a request body (capped at 1 MiB) as utf8 text. */
        ...
        disposeRpcRoute = webServer.register({
            ...
            handler: async (req, res) => {
                ...
                let endpoint;
                try {
                    endpoint = endpointFromPath(new URL(req.url ?? '/', 'http://x').pathname);
                }
                catch { /* malformed url */ }
                if (endpoint === undefined || body.method !== endpoint) {
                    rpcResponse(res, 400, {
                        type: 'server-response',
                        rpcId: body.rpcId,
                        result: { ok: false, error: { code: 'gateway/bad-request', message: `method ${JSON.stringify(body.method)} does not match endpoint ${JSON.stringify(endpoint)}`, details: {} } }
                    });
                    return;
                }
                try {
                    const result = await rpcHandler(endpoint, body.payload);
                    rpcResponse(res, 200, { type: 'server-response', rpcId: body.rpcId, result });
                }
                catch (error) {
                    res.writeHead(500);
